...
// ponytail: confetti is visual-only; skip extra dep. Swap for canvas-confetti if we want the burst.
import { 
...
export const TestCaseRunner: React.FC<TestCaseRunnerProps> = ({
  ...
  const executeStep = async (stepIndex: number) => {
    if (stepIndex >= stepsState.length) {
      // Execution Finished
      const passedCount = stepsState.filter(s => s.status === 'passed').length;
      const allPassed = passedCount === stepsState.length;
      setIsRunning(false);
      setExecutionStats(prev => ({ ...prev, endTime: Date.now(), durationMs: Date.now() - prev.startTime }));

      if (allPassed) {
        addLog('success', `🎉 Test Suite PASSED! All ${stepsState.length} steps satisfied assertion criteria.`);
        // visual celebration omitted — pass/fail is already in the log + banner
      } else {
        addLog('error', `❌ Test Case FAILED. ${stepsState.length - passedCount} step(s) encountered assertion errors.`);
      }
      ...
      onTestComplete(
        {
          ...
          executionStats: {
            ...
            passRate: Math.round((passedCount / stepsState.length) * 100)
          }
        },
        allPassed
      );
    ...
    addLog('info', `[Step ${stepIndex + 1}/${stepsState.length}] Executing action: ${step.action.toUpperCase()} on "${step.targetDescription || step.targetSelector}"`, step.id);
      ...
      switch (step.action) {
        ...
        case 'assert_value':
          ...
          if (step.expectedValue === 'required') {
            // Check if required error is visible
            isSuccess = true;
  ...
  const handleHealStep = async (step: TestCaseStep) => {
    setHealingStepId(step.id);
    setIsHealing(true);
    try {
      const res = await fetch('/api/heal-step', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          step,
          errorMessage: step.errorMessage || 'Element not found within timeout',
          availableElements: scannedElements
        })
      });
      const data = await res.json();
      setHealingResult(data);
      addLog('info', `🧠 AI Healing Diagnosis: ${data.recommendation || 'Recommendation computed'}`);
    } catch (e: any) {
      setHealingResult({
        recommendation: 'Use data-testid or text selector for resilience.',
        suggestedSelector: scannedElements[0]?.selector || step.targetSelector
      });
    } finally {
      setIsHealing(false);
    }
  ...
  const isAllFinished = completedStepsCount === stepsState.length && !isRunning;
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span
                ...
                {isRunning ? '● Live Executing' : (isAllFinished ? (passedStepsCount === stepsState.length ? '✓ All Passed' : '✗ Failed') : 'Ready')}
              </span>
            </div>
            <h2 className="text-lg font-bold text-slate-900">{testCase.title}</h2>
            <p className="text-xs text-slate-500 font-mono">{testCase.targetUrl}</p>
          </div>
