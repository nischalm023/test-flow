# Project Rules for AI Agents (Cursor)
...
**API mocking is mandatory.** Never hit the real backend (`localhost:3000`). Use helpers from `src/utils/mock-auth-api.ts`:

- `mockLoginSuccess(page, user, token)` — successful login
- `mockRegisterSuccess(page, user)` — successful registration
- `mockAuthError(page, 'login' \| 'register', message, status?)` — API failure

Test plan reference: `specs/auth-login-register.md`. Test data: `tests/data/auth.json`.
...
- Mock all client API calls with `page.route()` — never call the real backend in tests

## API mocking (EventFlow client)

When testing `client/` auth flows, block real API calls before any form submission:
...
Register mocks before navigating if the page fires requests on load. UI-only scenarios (validation errors) need no mock.
